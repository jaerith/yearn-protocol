...
contract WonkaEngine is ERC2746 {
    ...
    /// @dev Defines an event that will report when a ruletree has been invoked to validate a provided record.  Removed due to being unnecessary.
    /// @notice 
    // event CallRuleTree(
    ...
    bytes32 constant CONST_FIVE_ZERO_HASH = keccak256(abi.encodePacked("000000"));
    bytes32 constant CONST_Y_FLAG_HASH    = keccak256(abi.encodePacked("YN"));
    ...
    function addRuleTree(address _owner, bytes32 _ruleTreeName, string memory _desc) public onlyEngineOwner override {
        addRuleTree(_owner, _ruleTreeName, _desc, false, true, false);
    }
    ...
    /// @dev Default implementation for interface
    function addRule(address _owner, bytes32 _ruleSetName, bytes32 _ruleName, bytes32 _attrName, uint _ruleType, string memory _rightHandValue, bool _notFlag) public override {

        // NOTE: Need to implement function to search for RuleSetId here, based on "_ruleSetName"
        bytes32 ruleSetId = _ruleSetName;

        addRule(_owner, ruleSetId, _ruleName, _attrName, _ruleType, _rightHandValue, _notFlag, true);
    }
    ...
    /// @dev Default implementation for interface
    function executeRuleTree(address _owner) public override returns (bool) {

         return execute(_owner);
    }
    ...
    function executeWithReport(address ruler, WonkaRule storage targetRule, WonkaRuleReport memory ruleReport) private returns (bool ruleResult) {
        ...
        if (targetRule.targetAttr.isNumeric) {
            ...
            else if (keccak256(abi.encodePacked(targetRule.ruleValue)) == CONST_FIVE_ZERO_HASH) {
        ...
        if (almostOpInd) {
            ...
            ruleResult = (keccak256(abi.encodePacked(targetRule.ruleValueDomain[tempValue])) == CONST_Y_FLAG_HASH);
    ...
    /// @dev Default implmentation for interface
    function getRuleProps(address _owner, bytes32 _ruleSetName, uint _ruleIdx) public override returns (bytes32, uint, bytes32, string memory, bool, bytes32[] memory) {
        
        // NOTE: Need to implement function to search for RuleSetId here, based on "_ruleSetName"
        bytes32 ruleSetId = _ruleSetName;

        return getRuleProps(_owner, ruleSetId, true, _ruleIdx);
    }
    ...
    function getRuleSetPropsOriginal(address ruler, bytes32 rsId) public view returns (string memory, bool, bool, uint, uint, uint) {
        ...
        /// @dev This method will return the data that composes a particular RuleSet
    function getRuleSetProps(address ruler, bytes32 rsId) public view override returns (string memory, bool, bool, uint, uint, bytes32[] memory) {

        require(ruletrees[ruler].isValue == true, "The specified RuleTree does not exist.");

        return (ruletrees[ruler].allRuleSets[rsId].description, ruletrees[ruler].allRuleSets[rsId].severeFailure, ruletrees[ruler].allRuleSets[rsId].andOp, ruletrees[ruler].allRuleSets[rsId].evalRuleList.length, ruletrees[ruler].allRuleSets[rsId].assertiveRuleList.length, ruletrees[ruler].allRuleSets[rsId].childRuleSetList);
    }
